        ...
        <div className="container mx-auto px-4 max-w-3xl relative z-10">
          ...
          >
            ...
            <div className="text-center mb-8">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-muted border border-border text-primary text-sm font-medium  mx-auto mb-4">
                <UserIcon className="w-4 h-4" />
                حسابي الشخصي
              </div>
              <h1 className="text-3xl md:text-4xl font-black text-foreground tracking-tight">
                مرحباً بك، <span className="text-transparent bg-clip-text text-primary">{profile?.full_name?.split(' ')[0] || "يا صديقي"}</span>
              </h1>
            </div>
            ...
            {/* ======================================= */}
            <div className="bg-muted border border-border rounded-3xl p-8 flex flex-col items-center gap-5 shadow-xl relative overflow-hidden">

              <div className="relative z-10">
                <div className="w-28 h-28 rounded-full bg-muted flex items-center justify-center overflow-hidden border-4 border-background  ring-1 ring-border">
                  {uploading ? (
                    <div className="w-8 h-8 border-4 border-primary/20 border-t-emerald-500 rounded-full animate-spin" />
                  ) : profile?.avatar_url ? (
                    <img src={profile.avatar_url} alt="avatar" className="w-full h-full object-cover" />
                  ) : (
                ...
                <label className="absolute bottom-0 left-0 w-10 h-10 rounded-full bg-primary border-4 border-background text-primary-foreground flex items-center justify-center cursor-pointer hover:bg-primary/90 transition-colors shadow-lg">
                  <Camera className="w-4 h-4" />
                  <input type="file" accept="image/*" className="hidden" onChange={handleAvatarUpload} disabled={uploading} />
                </label>
              </div>
              ...
              <div className="text-center z-10">
                <div className="flex items-center justify-center gap-2 text-muted-foreground bg-muted border border-border px-4 py-2 rounded-xl text-sm">
            ...
            {/* ======================================= */}
            <div className="bg-muted border border-border rounded-3xl p-8 space-y-6 shadow-xl">
              <h2 className="text-xl font-bold text-foreground mb-2">المعلومات الأساسية</h2>
              
              <div className="space-y-5">
                <div>
                  <label className="text-sm font-bold text-muted-foreground mb-2 block">الاسم بالكامل</label>
                  <div className="relative group">
                    <UserIcon className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground group-focus-within:text-primary transition-colors" />
                    <input
                      type="text"
                      value={fullName}
                      onChange={(e) => setFullName(e.target.value)}
                      placeholder="أدخل اسمك الكامل"
                      className="w-full pr-12 pl-4 py-3.5 rounded-xl bg-muted border border-border text-foreground text-base placeholder:text-muted-foreground focus:outline-none focus:border-primary/20 focus:ring-1 focus:ring-primary transition-all shadow-inner"
                    />
                  </div>
                </div>

                <div>
                  <label className="text-sm font-bold text-muted-foreground mb-2 block">رقم الهاتف</label>
                  <div className="relative group">
                ...
                <Button 
                  onClick={handleSave} 
                  disabled={loading} 
                  className="w-full bg-primary hover:bg-primary/90 text-primary-foreground h-14 text-base font-bold rounded-xl transition-all  hover: flex items-center justify-center gap-2 mt-2"
                >
                  {loading ? (
            ...
            {/* ======================================= */}
            {certificates && certificates.length > 0 && (
              <div className="bg-muted border border-border rounded-3xl p-8 space-y-5 shadow-xl">
                <h2 className="text-xl font-bold text-foreground flex items-center gap-3 mb-2">
                ...
                <div className="space-y-3">
                  {certificates.map((cert) => (
                    <div key={cert.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-2xl bg-muted border border-border hover:border-primary/20 transition-all group">
                      <div>
                        <p className="font-bold text-foreground group-hover:text-primary transition-colors">{cert.courses?.title}</p>
                        <p className="text-xs text-muted-foreground mt-1 font-sans tracking-wider" dir="ltr">ID: {cert.certificate_number}</p>
                      </div>
                      <Button
                        size="sm"
                        className="bg-muted border border-border hover:border-primary text-foreground hover:text-foreground hover:bg-primary gap-2 rounded-lg h-10 w-full sm:w-auto transition-all"
                        onClick={() => handleDownloadCert(cert)}
                      >
                        <Download className="w-4 h-4" />
                        تحميل PDF
