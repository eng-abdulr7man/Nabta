...
const SubjectMaterialsPage = () => {
  ...
  const config = {
    ...
    section: { name: "السكاشن العملي", icon: PlaySquare, color: "text-primary", bg: "bg-accent", border: "border-primary/20" },
  ...
  }[type as keyof typeof config] || { name: "الملفات", icon: FileText, color: "text-foreground", bg: "bg-muted", border: "border-white/20" };

  return (
    <div className="min-h-screen flex flex-col bg-background font-tajawal selection:bg-accent" dir="rtl">
      <Navbar />
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-10 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              <button onClick={() => navigate("/library")} className="group w-12 h-12 rounded-2xl bg-muted border border-border flex items-center justify-center hover:border-primary/20 transition-all shadow-xl">
                <ArrowRight className="w-6 h-6 group-hover:text-primary transition-colors" />
              </button>
              <div>
                <h1 className="text-2xl md:text-4xl font-black text-foreground">{subject?.name || "..."}</h1>
          ...
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <p className="text-muted-foreground animate-pulse">جاري تحضير الملفات...</p>
            </div>
          ) : (
                ...
                {materials.length > 0 ? (
                  materials.map((mat, idx) => (
                    <motion.div
                      ...
                      className="relative group p-1 rounded-[2rem] bg-gradient-to-br from-white/5 to-transparent border border-border hover:border-primary/20 transition-all duration-500"
                    >
                      <div className="bg-muted rounded-[1.9rem] p-5 md:p-6 h-full flex flex-col">
                        <div className="flex items-start justify-between mb-6">
                          ...
                          <Sparkles className="w-5 h-5 text-neutral-800 group-hover:text-primary/40 transition-colors" />
                        </div>
                        ...
                        <div className="mt-auto">
                          <a href={mat.drive_url} target="_blank" rel="noopener noreferrer">
                            <Button className="w-full bg-primary hover:bg-primary/90 text-primary-foreground rounded-2xl h-14 text-base font-black shadow-lg  flex items-center gap-3 transition-all active:scale-95">
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="col-span-full py-20 bg-muted border-2 border-dashed border-border rounded-[3rem] flex flex-col items-center justify-center text-center px-6">
                    <div className="w-20 h-20 bg-muted rounded-full flex items-center justify-center mb-6">
                      <config.icon className="w-10 h-10 text-neutral-700" />
                    </div>
                    <h3 className="text-xl font-bold text-foreground mb-2">القسم فارغ حالياً</h3>
                    <p className="text-muted-foreground max-w-sm">سيقوم الأدمن برفع {config.name} في أقرب وقت ممكن. انتظرونا!</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
