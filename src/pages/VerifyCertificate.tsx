...
const VerifyCertificate = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4 font-tajawal relative overflow-hidden">
      ...
      >
        <div className="bg-muted border border-border rounded-[2rem] overflow-hidden shadow-2xl shadow-black">
          <div className="h-1.5 text-primary w-full" />
          
          <div className="p-8 lg:p-12">
            <div className="flex justify-between items-center mb-12">
              <div className="text-right">
                <h1 className="text-2xl font-black text-foreground">توثيق الشهادات</h1>
                <p className="text-primary/60 text-xs font-bold tracking-widest uppercase mt-1">Nabta Smart Verification</p>
              </div>
              <div className="w-14 h-14 bg-accent rounded-2xl flex items-center justify-center border border-primary/20 shadow-inner">
            ...
            {isLoading ? (
              <div className="py-24 text-center">
                ...
                <p className="text-muted-foreground text-sm animate-pulse">جاري فحص قاعدة البيانات الرقمية...</p>
              </div>
            ) : data ? (
              <div className="space-y-10">
                <div className="flex items-center gap-5 p-5 bg-white/[0.02] rounded-2xl border border-white/[0.05]">
                  ...
                  <div>
                    <h3 className="font-bold text-foreground text-lg">مستند رسمي معتمد</h3>
                    <p className="text-sm text-muted-foreground">تم التحقق من ملكية الشهادة وصحة البيانات</p>
                  </div>
                </div>
                ...
                {/* Action Bar */}
                <div className="pt-10 border-t border-border flex flex-col lg:flex-row items-center justify-between gap-8">
                  <div className="group relative w-full lg:w-auto">
                    ...
                    <button 
                      ...
                      className="relative flex items-center justify-between gap-6 px-5 py-3 bg-muted border border-border rounded-2xl hover:border-primary/20 transition-all w-full"
                    >
                      <div className="flex flex-col items-start">
                        <span className="text-[9px] text-primary/50 font-black uppercase tracking-[0.2em] mb-1">Blockchain ID</span>
                      ...
                      <div className="w-10 h-10 rounded-xl bg-accent flex items-center justify-center border border-primary/20 group-hover:bg-accent transition-all">
                  ...
                  <Button 
                    ...
                    className="relative overflow-hidden bg-primary hover:bg-primary/90 text-primary-foreground rounded-2xl px-12 h-16 font-bold transition-all shadow-2xl  group w-full lg:w-auto"
                  >
              ...
              <div className="py-16 text-center space-y-6">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-foreground">الشهادة غير صالحة</h2>
                  <p className="text-muted-foreground mt-2 max-w-[300px] mx-auto text-sm leading-relaxed">
                    لم يتم العثور على سجلات مطابقة. يرجى التأكد من أنك قمت بنسخ الرابط بشكل صحيح.
                  </p>
                </div>
                <Button variant="ghost" className="text-primary hover:bg-accent" onClick={() => window.location.href = '/'}>العودة للرئيسية</Button>
              </div>
            )}
          </div>
        </div>

        <p className="text-center mt-10 text-muted-foreground text-[10px] uppercase tracking-[0.3em] font-medium">
...
const DetailItem = ({ icon, label, value, isMono = false }: { icon: any, label: string, value: string, isMono?: boolean }) => (
  <div className="space-y-3 text-right group">
    <div className="flex items-center justify-start gap-2.5 text-muted-foreground group-hover:text-primary transition-colors">
      <span className="opacity-70">{icon}</span>
      <span className="text-[10px] font-black uppercase tracking-[0.15em]">{label}</span>
    </div>
    <p className={`text-neutral-200 font-bold text-lg leading-tight ${isMono ? 'font-mono tracking-tighter text-primary/90' : ''}`}>
      {value}
    </p>
  </div>
);
