...
const AdminDashboard = () => {
  ...
  const statCards = [
    ...
    { label: "الكورسات النشطة", value: stats?.activeCourses || 0, icon: BookOpen, color: "text-primary", bg: "bg-accent", border: "border-primary/20" },
    ...
    <AdminLayout>
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 p-2 font-tajawal relative overflow-x-hidden" dir="rtl">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1 bg-muted p-6 md:p-8 rounded-[2rem] border border-border shadow-xl relative overflow-hidden flex flex-col justify-center">
            <div className="absolute top-0 right-0 w-40 h-40 bg-accent blur-3xl -z-10" />
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-3xl md:text-4xl font-black text-foreground flex items-center gap-3">
                  {greeting}، <span className="text-primary">{profile?.full_name?.split(' ')[0] || "يا هندسة"}!</span> 👋
                </h1>
                <p className="text-muted-foreground font-medium mt-3 text-sm md:text-base">ملخص أداء منصة "نبتة". الأمور تسير بشكل رائع.</p>
              </div>
              {/* زرار التصدير للديسكتوب */}
              <Button onClick={handleExportReport} variant="outline" className="hidden md:flex gap-2 bg-muted border-border hover:bg-muted hover:text-foreground text-foreground rounded-xl h-12">
                <Download className="w-4 h-4" /> تصدير تقرير الأداء
              </Button>
            </div>
          </div>

          <div className="lg:w-1/3 flex flex-col justify-center gap-3 bg-background border border-border p-5 rounded-[2rem]">
            <h3 className="text-xs font-black text-muted-foreground px-2 uppercase tracking-widest flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Button onClick={() => navigate('/admin/courses')} className="bg-accent hover:bg-accent text-primary border border-primary/20 h-12 rounded-xl flex gap-2">
                <Plus className="w-4 h-4" /> كورس جديد
              </Button>
              {/* زرار التصدير للموبايل */}
              <Button onClick={handleExportReport} className="md:hidden col-span-2 bg-muted hover:bg-muted text-foreground border border-border h-12 rounded-xl flex gap-2">
                <Download className="w-4 h-4" /> تصدير تقرير الأداء
              </Button>
            </div>
          </div>
          ...
          {statCards.map((card, i) => {
            ...
            return (
              <motion.div key={card.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                className={`bg-muted border border-border p-5 rounded-3xl shadow-xl hover:border-border transition-colors relative overflow-hidden group`}
              >
                ...
                <div className="relative z-10 space-y-1">
                  <p className="text-xl md:text-2xl font-black text-foreground">{card.value}</p>
                  <p className="text-xs font-bold text-muted-foreground line-clamp-1">{card.label}</p>
                </div>
              </motion.div>
            );
          })}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="lg:col-span-2 bg-muted border border-border rounded-[2.5rem] p-6 shadow-xl">
            ...
            {(monthlyData || []).length > 0 ? (
              ...
              <div className="h-[280px] flex items-center justify-center text-muted-foreground font-bold">لا توجد بيانات كافية</div>
            )}
          </motion.div>

          {/* 🚨 المهام العاجلة (Needs Attention) 🚨 */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="lg:col-span-1 bg-muted border border-red-500/20 rounded-[2.5rem] p-6 shadow-xl flex flex-col relative overflow-hidden">
            ...
            <div className="flex flex-col gap-4">
              <Link to="/admin/messages" className="flex items-center justify-between p-4 bg-muted border border-border rounded-2xl hover:border-red-500/30 transition-colors group">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-red-500/10 rounded-lg text-red-500 group-hover:bg-red-500 group-hover:text-foreground transition-colors"><MessageSquare className="w-4 h-4" /></div>
                  <span className="text-sm font-bold text-foreground">رسائل دعم غير مقروءة</span>
                </div>
                <span className="font-black text-red-500">{stats?.unreadMessages || 0}</span>
              </Link>
              
              <Link to="/admin/courses" className="flex items-center justify-between p-4 bg-muted border border-border rounded-2xl hover:border-amber-500/30 transition-colors group">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-amber-500/10 rounded-lg text-amber-500 group-hover:bg-amber-500 group-hover:text-foreground transition-colors"><FileWarning className="w-4 h-4" /></div>
                  <span className="text-sm font-bold text-foreground">كورسات مسودة (غير منشورة)</span>
                </div>
                <span className="font-black text-amber-500">{stats?.draftCourses || 0}</span>
              </Link>

              <div className="flex items-center justify-between p-4 bg-muted border border-border rounded-2xl opacity-60 cursor-not-allowed">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-muted rounded-lg text-muted-foreground"><Star className="w-4 h-4" /></div>
                  <span className="text-sm font-bold text-muted-foreground">تقييمات تحتاج مراجعة</span>
                </div>
                <span className="font-black text-muted-foreground">0</span>
              </div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-muted border border-border rounded-[2rem] p-6 shadow-xl">
            <h3 className="font-black text-foreground mb-6 flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-primary" /> الكورسات الأكثر إقبالاً (Progress)
            </h3>
                ...
                {courseEnrollments?.map((course: any, idx: number) => (
                  <div key={idx} className="space-y-2.5">
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-foreground font-bold truncate pr-2 max-w-[70%]">{course.name}</span>
                      <span className="text-primary font-black">{course.طلاب} <span className="text-muted-foreground text-xs">طالب</span></span>
                    </div>
                    <div className="w-full bg-muted rounded-full h-3.5 border border-border overflow-hidden shadow-inner">
                      <motion.div initial={{ width: 0 }} animate={{ width: `${(course.طلاب / maxEnrollments) * 100}%` }} transition={{ duration: 1, delay: idx * 0.1, ease: "easeOut" }}
                        className="text-primary h-full rounded-full  relative"
                      >
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="bg-muted border border-border rounded-[2rem] p-6 shadow-xl flex flex-col">
              ...
              <div className="flex-1 flex flex-col justify-between">
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                  {specDistribution?.map((spec: any, idx: number) => (
                    <div key={idx} className="flex items-center gap-2.5 bg-muted p-2.5 rounded-xl border border-border hover:border-border transition-colors">
                      <div className="w-3 h-3 rounded-full shrink-0 shadow-md" style={{ backgroundColor: CHART_COLORS[idx % CHART_COLORS.length] }} />
                      <span className="text-[11px] font-bold text-foreground truncate flex-1">{spec.name}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="bg-muted border border-amber-500/20 rounded-[2.5rem] p-6 shadow-xl flex flex-col relative overflow-hidden">
            ...
            <div className="flex flex-col gap-3">
              {topLearners && topLearners.length > 0 ? (
                topLearners.map((learner: any) => {
                  ...
                  const medalColors = ["bg-amber-400 text-amber-950", "bg-neutral-300 text-neutral-800", "bg-orange-700 text-orange-100", "bg-muted text-muted-foreground"];
                  const ringColors = ["ring-amber-500/30", "ring-border", "ring-orange-700/30", "ring-transparent"];
                  
                  return (
                    <div key={learner.rank} className={`flex items-center justify-between p-4 rounded-2xl bg-muted border border-border hover:border-amber-500/30 transition-all ${learner.rank === 1 ? 'ring-1 ring-amber-500/50 shadow-[0_0_15px_rgba(245,158,11,0.1)]' : ''}`}>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-sm font-bold text-foreground truncate">{learner.profile?.full_name || "مستخدم مجهول"}</p>
                          <p className="text-[11px] text-amber-500/80 font-bold">بطل التعلم</p>
                        </div>
                      </div>
                      <div className="text-center bg-muted px-4 py-2 rounded-xl border border-border">
                        <p className="text-sm font-black text-amber-400">{learner.enrollmentCount}</p>
                        <p className="text-[9px] text-muted-foreground font-bold">كورسات</p>
                      </div>
                    </div>
                  );
                })
              ) : (
                <div className="py-10 text-center text-muted-foreground font-bold text-sm">لا توجد بيانات كافية للطلاب حتى الآن</div>
              )}
            </div>
          </motion.div>

          {/* 📡 الرادار المباشر (Live Feed) 📡 */}
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} className="bg-muted border border-blue-500/20 rounded-[2.5rem] p-6 shadow-xl flex flex-col min-h-[380px] relative overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto pr-1 custom-scrollbar space-y-3">
              {recentActivity && recentActivity.length > 0 ? (
                recentActivity.map((act: any) => (
                  <div key={act.id} className="bg-muted p-4 rounded-2xl border border-border hover:border-blue-500/30 transition-colors group cursor-default">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500/20 to-emerald-500/20 border border-blue-500/30 text-foreground flex items-center justify-center font-black text-sm shrink-0 shadow-inner group-hover:scale-110 transition-transform">
                          {act.profile?.full_name?.charAt(0) || "U"}
                        </div>
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-foreground truncate">{act.profile?.full_name || "مستخدم جديد"}</p>
                          <p className="text-[11px] text-muted-foreground font-medium truncate mt-0.5">سجل في <span className="text-blue-400 font-bold">{act.course?.title || "كورس محذوف"}</span></p>
                        </div>
                      </div>
                      <div className="flex flex-col items-end gap-1 text-[10px] text-muted-foreground font-medium shrink-0">
