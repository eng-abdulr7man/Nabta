            ...
            <div className="mt-auto pt-10 pb-6 space-y-4">
              {user ? (
                <div className="p-5 rounded-3xl bg-[#0a0f0c] border border-neutral-800 shadow-2xl">
                  ...
                  <Button onClick={() => { signOut(); setIsOpen(false); }} variant="outline" className="w-full h-14 rounded-2xl border-red-500/30 text-red-500 font-bold gap-2">
                    <LogOut className="w-5 h-5" /> تسجيل الخروج
                  </Button>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  <Link to="/login" onClick={() => setIsOpen(false)}><Button variant="outline" className="w-full h-14 rounded-2xl border-neutral-700 text-white text-lg font-bold">دخول</Button></Link>
                  <Link to="/register" onClick={() => setIsOpen(false)}><Button className="w-full h-14 rounded-2xl bg-emerald-600 text-white text-lg font-bold">حساب جديد</Button></Link>
                </div>
              )}
            </div>
          </motion.div>
