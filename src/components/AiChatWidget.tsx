...
import { 
  ...
  CheckCircle2, MessageSquarePlus, Gift 
} from "lucide-react";
    ...
    try {
      ...
      const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: "system", content: `أنت خبير زراعي في نبتة. قائمة منتجاتنا: (${storeInventory}). انصح بها إذا كانت مناسبة للحل.` },
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: "حصل دروب بسيط، جرب تاني يا هندسة." }]);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          >
            ...
            <div className="p-6 bg-[#121A15] border-b border-white/5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="font-bold text-white text-sm">مستشار نبتة</h4>
                ...
                return (
                  <div key={i} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                    <div className={`max-w-[85%] p-4 rounded-[1.5rem] text-sm leading-relaxed shadow-sm ${
                      msg.role === "user" ? "bg-emerald-600 text-white rounded-bl-none" : "bg-white/5 text-neutral-200 rounded-br-none border border-white/5"
                    }`}>
                      {msg.content}
                    </div>
                    ...
                    {product && (
                      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
                        className="mt-3 w-full bg-[#121A15] border border-emerald-500/20 rounded-2xl p-3 flex items-center gap-3"
                      >
                        <div className="w-14 h-14 rounded-xl overflow-hidden bg-black/40 border border-white/5"><img src={product.image_url} className="w-full h-full object-cover" /></div>
                        <div className="flex-1 min-w-0"><h5 className="text-xs font-bold text-white truncate">{product.name}</h5><p className="text-emerald-400 font-black text-sm">{product.price}</p></div>
                        <a href={`https://wa.me/${WHATSAPP_NUMBER}?text=محتاج طلب من متجر نبتة: ${product.name}`} className="bg-emerald-600 text-white p-2.5 rounded-xl shadow-lg hover:bg-emerald-500 transition-all"><ShoppingBag size={16} /></a>
                      </motion.div>
                    )}
                    ...
                    {msg.showActions && !isLoading && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 flex flex-wrap gap-2 justify-start w-full">
                        ...
                        <button 
                          onClick={() => setInput("عندي سؤال تاني..")}
            ...
            <div className="p-4 bg-white/5 border-t border-white/5">
              <div className="relative flex items-center">
                ...
                />
                <button onClick={() => handleSendMessage()} disabled={isLoading} className="absolute left-3 top-1/2 -translate-y-1/2 text-emerald-500 disabled:opacity-30 hover:text-white transition-colors"><Send size={20} /></button>
              </div>
            </div>
          </motion.div>
        )}
