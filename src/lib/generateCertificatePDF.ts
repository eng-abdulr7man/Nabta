...
export const generateProfessionalCertificatePDF = async (
  ...
  doc.setFontSize(32);
  doc.setTextColor(255, 255, 255);
  doc.text("CERTIFICATE OF COMPLETION", w / 2, 55, {
    align: "center",
  });
  ...
  doc.text("This is proudly awarded to", w / 2, 75, {
    align: "center",
  });
  ...
  doc.setFontSize(30);
  doc.setTextColor(255, 255, 255);
  doc.text(data.learnerName, w / 2, 95, {
    align: "center",
  });
  ...
  doc.setFontSize(22);
  doc.setTextColor(120, 200, 140);
  doc.text(data.courseName, w / 2, 130, {
    align: "center",
  });
  ...
  doc.setTextColor(180, 200, 190);
  doc.text(
    data.instructor ? data.instructor : "Course Instructor",
    65,
    signatureY + 6,
    { align: "center" }
  );
  ...
  doc.setFontSize(9);
  doc.text(`Date: ${formattedDate}`, 40, h - 25);
  doc.text(
  ...
  doc.circle(w - 55, 45, 14, "F");

  doc.setTextColor(12, 20, 18);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text("VERIFIED", w - 55, 47, { align: "center" });
...
export const downloadProfessionalCertificate = async (
  data: CertificateData
) => {
  const doc = await generateProfessionalCertificatePDF(data);
  doc.save(`certificate-${data.certificateNumber}.pdf`);
};
